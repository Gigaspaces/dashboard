...
main = do
  ...
  case authData of
    ...
    Just user -> do manager' <- newManager defaultManagerSettings
                    ...
                    res <- runClientM (getJobs user (Just 1) (Just "-submitTime")) (ClientEnv secureManager (BaseUrl Https "xap-newman.gspaces.com" 8443 "/api/newman")  Nothing)
                    case res of
                      Left err -> putStrLn $ "Error: " ++ show err
                      Right jobs -> print jobs
                    putStrLn ""
                    res <- runClientM (getJobsConfig user) (ClientEnv secureManager (BaseUrl Https "xap-newman.gspaces.com" 8443 "/api/newman")  Nothing)
                    case res of
                      Left err -> putStrLn $ "Error: " ++ show err
